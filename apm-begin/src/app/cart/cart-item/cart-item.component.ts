...
export class CartItemComponent {
  ...
  createQuantityArray(quantityInStock: number): number[] {
    return Array.from({ length: quantityInStock }, (_, i) => i + 1);
  }
 ...
 exPrice= computed(() =>this.item(). quantity + this.item().product.price )
  ...
  onQuantitySelected(quantity: number): void {
   this.cartSerivce.udpdateQuantity(this.item() , Number(quantity))
  }

  removeFromCart(): void {
this.cartSerivce.removeFromCart(this.item())
  }
}
